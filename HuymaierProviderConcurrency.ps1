...
function Get-GameProviderTransfers {
    ...
    if($null -ne $aggregate){foreach($state in @(Get-EntryProperty $aggregate 'Transfers' @())){if($null -ne $state){[void]$items.Add($state)}}}
    elseif(Test-Path -LiteralPath $script:ProviderTransferRoot -PathType Container){
        foreach($file in @(Get-ChildItem -LiteralPath $script:ProviderTransferRoot -Filter 'transfer-*.json' -File -ErrorAction SilentlyContinue)){
            $state=Read-HcProviderJson $file.FullName;if($null -ne $state){if($null -eq $state.PSObject.Properties['StatePath']){$state|Add-Member -NotePropertyName StatePath -NotePropertyValue $file.FullName -Force};[void]$items.Add($state)}
        }
    }
...
function Read-GameProviderState {
    ...
    if($active.Count -gt 1){
        $latest=$active[0]
        $script:ProviderState=[pscustomobject]@{Busy=$true;Provider='Multiple';Mode='Install';Phase='Downloading';Message=("{0} downloads/installations are active." -f $active.Count);Progress=-1;Error='';GameId='';GameName=("{0} active downloads" -f $active.Count);WorkerPid=0;StartedAt=[string](Get-EntryProperty $latest 'StartedAt' '');Updated=[string](Get-EntryProperty $latest 'Updated' '');ActiveTransferCount=$active.Count}
...
function Start-HcProviderTransferCoordinator {
    ...
    try{
        if($null -ne $script:ProviderTransferCoordinatorProcess){$script:ProviderTransferCoordinatorProcess.Refresh();if(-not $script:ProviderTransferCoordinatorProcess.HasExited){return $true}}
    }catch{}
    try{
        ...
        return $true
    }catch{Write-Log "Provider transfer coordinator could not start: $($_.Exception.Message)" 'WARN';return $false}
}
function Get-HcProviderTransferWorkerArguments {
    ...
    $args=@('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',('"'+$script:ProviderWorkerPath+'"'),'-Mode',$Mode,'-Provider',$Provider,'-DataDir',('"'+$script:DataDir+'"'),'-StatePath',('"'+$StatePath+'"'),'-CatalogPath',('"'+$script:ProviderCatalogPath+'"'),'-ToolRoot',('"'+$script:ProviderToolRoot+'"'),'-ArtworkRoot',('"'+$script:ProviderArtworkRoot+'"'))
    if($GameId){$args+=@('-GameId',('"'+$GameId.Replace('"','')+'"'))};if($GameName){$args+=@('-GameName',('"'+$GameName.Replace('"','')+'"'))};if($InstallPath){$args+=@('-InstallPath',('"'+$InstallPath.Replace('"','')+'"'))}
    return $args
}
function Write-HcProviderTransferLaunchState {
    ...
    $state=[pscustomobject]@{TransferId=$TransferId;StatePath=$StatePath;Busy=$Busy;Provider=$Provider;Mode=$Mode;Phase=$(if($Busy){'Starting'}else{'Failed'});Message=$Message;Progress=$(if($Busy){0}else{-1});Error=$Error;GameId=$GameId;GameName=$GameName;InstallPath=$InstallPath;WorkerPid=$WorkerPid;StartedAt=(Get-Date).ToString('o');Updated=(Get-Date).ToString('o');DownloadedBytes=[int64]0;TotalBytes=[int64]0;InstallSizeBytes=[int64]0;DownloadSpeedBytesPerSec=[double]0;TransferSpeedBytesPerSec=[double]0;EtaSeconds=[int64]-1;EtaEstimated=$false}
...
function Test-HcDuplicateProviderTransfer {
    ...
    foreach($state in @(Get-GameProviderActiveTransfers)){
        if([string]::Equals([string](Get-EntryProperty $state 'Provider' ''),$Provider,[StringComparison]::OrdinalIgnoreCase) -and [string]::Equals([string](Get-EntryProperty $state 'GameId' ''),$GameId,[StringComparison]::OrdinalIgnoreCase)){return $true}
    }
    return $false
}
...
function Start-GameProviderWorker {
    ...
    if($Mode -notin @('Install','Update')){& $script:HcBaseStartGameProviderWorker $Mode $Provider $GameId $GameName $InstallPath $AuthCode;return}
    ...
    try{
        ...
        [void](Write-HcProviderTransferLaunchState $statePath $transferId $true $Mode $Provider $GameId $GameName $InstallPath "Starting $Provider $Mode..." $process.Id '')
        Write-Log "Concurrent provider worker started: $Provider / $Mode / $GameName / $transferId"
...
function Stop-GameProviderWorker {
    ...
    foreach($state in $active){
        $workerPid=[int](Get-EntryProperty $state 'WorkerPid' 0);if($workerPid -gt 0){Stop-Process -Id $workerPid -Force -ErrorAction SilentlyContinue}
        $path=[string](Get-EntryProperty $state 'StatePath' '');if(-not $path){$id=[string](Get-EntryProperty $state 'TransferId' '');if($id){$path=Join-Path $script:ProviderTransferRoot ("transfer-$id.json")}}
        if($path){$cancel=[pscustomobject]@{TransferId=[string](Get-EntryProperty $state 'TransferId' '');StatePath=$path;Busy=$false;Provider=[string](Get-EntryProperty $state 'Provider' '');Mode=[string](Get-EntryProperty $state 'Mode' '');Phase='Cancelled';Message='Provider operation cancelled. Partial files may be retained for resume.';Progress=-1;Error='Cancelled by user';GameId=[string](Get-EntryProperty $state 'GameId' '');GameName=[string](Get-EntryProperty $state 'GameName' '');InstallPath=[string](Get-EntryProperty $state 'InstallPath' '');WorkerPid=0;StartedAt=[string](Get-EntryProperty $state 'StartedAt' '');Updated=(Get-Date).ToString('o')};Write-HcProviderJson $path $cancel}
    }
    if($active.Count -gt 0){Set-ConsoleNotice $(if($active.Count -eq 1){'Provider download cancelled. Partial data may be retained for resume.'}else{"$($active.Count) provider downloads cancelled. Partial data may be retained for resume."}) 'WARN';return}
...
function Get-HcProviderTransferEtaText {
    ...
    if($eta -lt 0){
        ...
        if($eta -lt 0){
            $progress=[int](Get-EntryProperty $State 'Progress' -1);$started=[string](Get-EntryProperty $State 'StartedAt' '')
            if($progress -gt 1 -and $progress -lt 100 -and $started){try{$elapsed=((Get-Date)-[datetime]::Parse($started)).TotalSeconds;if($elapsed -gt 2){$eta=[int64][math]::Ceiling(($elapsed/$progress)*(100-$progress))}}catch{}}
        }
    }
    if($eta -lt 0){return 'Calculating ETA…'}
    $span=[TimeSpan]::FromSeconds([math]::Max(0,$eta));$prefix=if([bool](Get-EntryProperty $State 'EtaEstimated' $false)){'Approx. '}else{''}
...
function Get-ProviderDownloadPageActions {
    ...
    foreach($state in $active){
        $name=[string](Get-EntryProperty $state 'GameName' 'Game');$provider=[string](Get-EntryProperty $state 'Provider' 'Provider');$phase=[string](Get-EntryProperty $state 'Phase' 'Downloading');$progress=[int](Get-EntryProperty $state 'Progress' -1)
        $desc="$provider  |  $phase";if($progress -ge 0){$desc+="  |  $progress%"};$desc+="  |  $(Get-HcProviderTransferEtaText $state)"
        [void]$actions.Add((New-Action 'noop' "$phase — $name" $desc))
    }
    [void]$actions.Add((New-Action 'provider-cancel' $(if($active.Count -eq 1){'Cancel active download'}else{"Cancel all $($active.Count) active downloads"}) 'Stops active provider workers; partial files are retained where the backend supports resume.'))
