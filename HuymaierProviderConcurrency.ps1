...
# Loaded after HuymaierGameProviders.ps1 so Install/Update can run together
# while setup/auth/refresh/move/uninstall keep the existing serialized path.
...
function Get-GameProviderTransfers {
    ...
    if($null -ne $aggregate){
        foreach($state in @(Get-EntryProperty $aggregate 'Transfers' @())){
            if($null -ne $state){[void]$items.Add($state)}
        }
    ...
    $sorted=@($items.ToArray()|Sort-Object {try{[datetime]::Parse([string](Get-EntryProperty $_ 'Updated' ''))}catch{[datetime]::MinValue}} -Descending)
...
function Read-GameProviderState {
    ...
    if($active.Count -gt 1){
        $latest=$active[0]
        $updated=[string](Get-EntryProperty $latest 'TelemetryUpdated' '')
        if(-not $updated){$updated=[string](Get-EntryProperty $latest 'Updated' '')}
        $script:ProviderState=[pscustomobject]@{
            Busy=$true
            Provider='Multiple'
            Mode='Install'
            Phase='Downloading'
            Message=([string]$active.Count+' downloads/installations are active.')
            Progress=-1
            Error=''
            GameId=''
            GameName=([string]$active.Count+' active downloads')
            WorkerPid=0
            StartedAt=[string](Get-EntryProperty $latest 'StartedAt' '')
            Updated=$updated
    ...
    if($active.Count -eq 1){
        $script:ProviderState=$active[0]
        return $script:ProviderState
    }
    $transfers=@(Get-GameProviderTransfers)
    if($transfers.Count -gt 0){
        $script:ProviderState=$transfers[0]
        return $script:ProviderState
    }
...
function Start-HcProviderTransferCoordinator {
    ...
    try{
        ...
        $workerArg='"'+$script:ProviderTransferCoordinatorPath+'"'
        $baseArg='"'+$script:BaseDir+'"'
        $dataArg='"'+$script:DataDir+'"'
        $args=@('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',$workerArg,'-ParentPid',[string]$PID,'-BaseDir',$baseArg,'-DataDir',$dataArg)
...
function Write-HcProviderTransferLaunchState {
    ...
    $phase='Failed'
    $progress=-1
    if($Busy){$phase='Starting';$progress=0}
    $state=[pscustomobject]@{
        ...
        DownloadSpeedBytesPerSec=[double]0
        TransferSpeedBytesPerSec=[double]0
        EtaSeconds=[int64]-1
        EtaEstimated=$false
    }
...
function Test-HcDuplicateProviderTransfer {
    ...
    foreach($state in @(Get-GameProviderActiveTransfers)){
        $sameProvider=[string]::Equals([string](Get-EntryProperty $state 'Provider' ''),$Provider,[StringComparison]::OrdinalIgnoreCase)
        $sameGame=[string]::Equals([string](Get-EntryProperty $state 'GameId' ''),$GameId,[StringComparison]::OrdinalIgnoreCase)
        if($sameProvider -and $sameGame){return $true}
    }
    return $false
}
...
function Start-GameProviderWorker {
    ...
    if(-not(Test-Path -LiteralPath $script:ProviderWorkerPath -PathType Leaf)){
        Set-ConsoleNotice 'The direct-provider worker is missing.' 'ERROR'
        return
    }
    if(Test-HcDuplicateProviderTransfer $Provider $GameId){
        Set-ConsoleNotice ($GameName+' already has an active '+$Provider+' transfer.') 'WARN'
        return
    }
    if(-not $InstallPath){$InstallPath=Get-ProviderInstallRoot $Provider}
    if(-not(Start-HcProviderTransferCoordinator)){
        Set-ConsoleNotice 'The provider transfer coordinator could not start.' 'ERROR'
        return
    }
    ...
    try{
        $powershell=$env:SystemRoot+'\System32\WindowsPowerShell\v1.0\powershell.exe'
        $workerArgs=Get-HcProviderTransferWorkerArguments $Mode $Provider $GameId $GameName $InstallPath $statePath
        $process=Start-Process -FilePath $powershell -ArgumentList $workerArgs -WindowStyle Hidden -PassThru
...
function Stop-GameProviderWorker {
    ...
    foreach($state in $active){
        ...
        if($workerPid -gt 0){
            ...
            try{
                $process=$script:ProviderWorkerProcesses[$transferId]
        ...
        if($path){
            $cancel=[pscustomobject]@{
                ...
                Phase='Cancelled'
                Message='Provider operation cancelled. Partial files may be retained for resume.'
                Progress=-1
                Error='Cancelled by user'
                GameId=[string](Get-EntryProperty $state 'GameId' '')
                GameName=[string](Get-EntryProperty $state 'GameName' '')
                InstallPath=[string](Get-EntryProperty $state 'InstallPath' '')
                WorkerPid=0
                StartedAt=[string](Get-EntryProperty $state 'StartedAt' '')
                Updated=(Get-Date).ToString('o')
            }
    ...
    if($active.Count -gt 0){
        $notice='Provider download cancelled. Partial data may be retained for resume.'
        if($active.Count -gt 1){$notice=[string]$active.Count+' provider downloads cancelled. Partial data may be retained for resume.'}
...
function Get-HcProviderTransferEtaText {
    ...
    if($eta -lt 0){
        ...
        if($installing){
            $current=[int64](Get-EntryProperty $State 'InstallProcessedBytes' 0)
            $total=[int64](Get-EntryProperty $State 'InstallSizeBytes' 0)
            $speed=[double](Get-EntryProperty $State 'InstallSpeedBytesPerSec' (Get-EntryProperty $State 'TransferSpeedBytesPerSec' 0))
        }else{
            $current=[int64](Get-EntryProperty $State 'DownloadedBytes' 0)
            $total=[int64](Get-EntryProperty $State 'TotalBytes' 0)
            $speed=[double](Get-EntryProperty $State 'DownloadSpeedBytesPerSec' (Get-EntryProperty $State 'TransferSpeedBytesPerSec' 0))
        }
        if($total -gt 0 -and $speed -gt 1){
            $remaining=[math]::Max(0,$total-$current)
            $eta=[int64][math]::Ceiling($remaining/$speed)
        }
        if($eta -lt 0){
    ...
    $prefix=''
    if([bool](Get-EntryProperty $State 'EtaEstimated' $false)){$prefix='Approx. '}
...
function Get-ProviderDownloadPageActions {
    ...
    $cancelTitle='Cancel active download'
    if($active.Count -gt 1){$cancelTitle='Cancel all '+$active.Count+' active downloads'}
