...
foreach($needle in @('HUYMAIER_D3D11_SHELF_SHADER_V4_COLOR_MANAGED_UI_PBR','SrgbToLinear','LinearToSrgb','environmentSpec','displayRgb=LinearToSrgb')){if($nativeAscii.IndexOf($needle,[StringComparison]::Ordinal)-lt0){throw "Staged native DLL omits color-managed shelf shader contract: $needle"}}
