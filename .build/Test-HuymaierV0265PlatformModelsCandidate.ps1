...
try{$glb=Join-Path $temp 'probe.glb';$cache=Join-Path $temp 'probe.hc3d';New-HcV7ProbeGlb $glb;$proc=Start-Process -FilePath (NeedFile 'HuymaierGpuShelfAssetCompiler.exe') -ArgumentList ('--model "'+$glb+'" --cache "'+$cache+'" --size 128') -WindowStyle Hidden -Wait -PassThru;if($proc.ExitCode-ne0-or-not(Test-Path $cache)){throw "Staged GPU cache compiler failed with exit $($proc.ExitCode)"};$br=New-Object IO.BinaryReader([IO.File]::OpenRead($cache));try{if((-join$br.ReadChars(4))-ne'HC3D'-or$br.ReadInt32()-ne2){throw 'Staged compiler produced invalid HC3D cache.'};[void]$br.ReadInt64();[void]$br.ReadInt64();if($br.ReadInt32()-ne228){throw 'Staged compiler ignored requested cache quality.'};$vc=$br.ReadInt32();$ic=$br.ReadInt32();$dc=$br.ReadInt32();$images=$br.ReadInt32();if($vc-ne4-or$ic-ne6-or$dc-ne1-or$images-ne0){throw "Unexpected staged HC3D counts: v=$vc i=$ic d=$dc images=$images"}}finally{$br.Dispose()}}finally{Remove-Item -LiteralPath $temp -Recurse -Force -ErrorAction SilentlyContinue}
