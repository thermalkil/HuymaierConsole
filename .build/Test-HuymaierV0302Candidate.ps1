...
function Require-File([string]$Relative){
    ...
    if(-not(Test-Path -LiteralPath $path -PathType Leaf)){throw "v0.30.2 required file missing: $Relative"}
    return $path
}
function Require-Text([string]$Relative,[string[]]$Needles){
    $path=Require-File $Relative
    $raw=Get-Content -Raw -LiteralPath $path -Encoding UTF8
    foreach($needle in $Needles){if($raw.IndexOf($needle,[StringComparison]::Ordinal) -lt 0){throw "$Relative missing v0.30.2 invariant: $needle"}}
    return $raw
}
...
if([string]$validation.version -ne '0.30.2'){throw "Candidate validation version is $($validation.version), expected 0.30.2."}
if([string]$validation.asset -ne 'HC0302.zip'){throw "Candidate validation asset is $($validation.asset), expected HC0302.zip."}
...
if([string]$manifest.version -ne '0.30.2'){throw 'Candidate manifest is not v0.30.2.'}
if([string]$manifest.baseVersion -ne '0.30.1'){throw 'v0.30.2 candidate does not identify v0.30.1 as its logical base.'}
if([string]$manifest.build -ne 'console-brightness-rc1'){throw 'v0.30.2 build identity is not console-brightness-rc1.'}

$core=Require-Text 'HuymaierConsole.ps1' @("`$script:AppVersion = '0.30.2'",'HUYMAIER_UNIFIED_CURSOR_RUNTIME_V2','HuymaierUser3DModels.ps1')
$bootstrap=Require-Text 'HuymaierBootstrap.ps1' @("`$script:ExpectedConsoleVersion='0.30.2'",'Assert-HuymaierInstallIntegrity','HuymaierUser3DModels.ps1')
$installerCore=Require-Text 'HuymaierInstallerCore.ps1' @("`$script:InstallVersion='0.30.2'",'install-incomplete.json')
$installer=Require-Text 'Install-HuymaierConsole.ps1' @("param([string]`$InstallRoot,[string]`$Version='0.30.2')","-Version '0.30.2'",'SilentUpdate')
$appx=Require-Text 'FSEPackage\AppxManifest.xml' @('Version="0.30.2.0"')

$customization=Require-Text 'HuymaierCustomization.ps1' @(
...
if(-not((Get-HcComparableVersion 'v0.30.2') -gt (Get-HcComparableVersion 'v0.30.1'))){throw 'Staged updater cannot order v0.30.2 above v0.30.1.'}
if(-not((Get-HcComparableVersion 'v0.30.2') -gt (Get-HcComparableVersion '0.26.2'))){throw 'Staged updater cannot order legacy v0.26.2 below v0.30.2.'}
...
foreach($relative in @('HuymaierConsole.ps1','HuymaierBootstrap.ps1','HuymaierInstallerCore.ps1','Install-HuymaierConsole.ps1','HuymaierConsoleUpdateWorker.ps1','HuymaierSelfUpdater.ps1','HuymaierShellRedesign.ps1','HuymaierUser3DModels.ps1','HuymaierCustomization.ps1')){Assert-Ps51Parse $relative}
...
if($nativeVersion -ne '0.30.2' -or $nativeArch -ne 'x64'){throw "Staged native build stamp mismatch: $nativeVersion/$nativeArch"}
...
foreach($forbidden in @('.development','.source','.release','.github','.build','Docs')){if(Test-Path -LiteralPath (Join-Path $StageRoot $forbidden)){throw "Developer-only directory survived v0.30.2 package: $forbidden"}}
if(@(Get-ChildItem -LiteralPath $StageRoot -File -Filter 'BUILD-VALIDATION*.txt' -ErrorAction SilentlyContinue).Count){throw 'Historical BUILD-VALIDATION files survived v0.30.2 package.'}

Write-Host 'v0302CandidateVersionIdentityGate: success'
Write-Host 'v0302CandidateConsoleBrightnessGate: success'
Write-Host 'v0302CandidateCarryForwardGate: success'
Write-Host 'v0302CandidateSelfUpdateGate: success'
Write-Host 'v0302CandidatePackageHygieneGate: success'