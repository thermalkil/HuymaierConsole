...
function Write-AtomicJson {param([string]$Path,$Value);try{$parent=Split-Path -Parent $Path;if($parent){New-Item -ItemType Directory -Force -Path $parent|Out-Null};$tmp="$Path.$PID.tmp";$Value|ConvertTo-Json -Depth 20|Set-Content -LiteralPath $tmp -Encoding UTF8;Move-Item -LiteralPath $tmp -Destination $Path -Force}catch{}}
function Quote-Arg {param([string]$Value);if($null -eq $Value){return '""'};return '"'+$Value.Replace('"','')+'"'}
function Get-TransferId {param([string]$Path,$State);$id=[string](Get-Prop $State 'TransferId' '');if($id){return $id};$leaf=[IO.Path]::GetFileNameWithoutExtension($Path);if($leaf -match '^transfer-(.+)$'){return [string]$matches[1]};return $leaf}

function Convert-SizeTextBytes {
    param([string]$Text)
    if([string]::IsNullOrWhiteSpace($Text)){return [int64]0}
    $m=[regex]::Match($Text,'(?i)([0-9]+(?:\.[0-9]+)?)\s*(B|KB|KiB|MB|MiB|GB|GiB|TB|TiB)')
...
function Get-ConfiguredInstallRoot {
    ...
    foreach($entry in @(Get-Prop $config 'ProviderInstallRoots' @())){if([string]::Equals([string](Get-Prop $entry 'Provider' ''),$Provider,[StringComparison]::OrdinalIgnoreCase)){$path=[string](Get-Prop $entry 'Path' '');if($path){return $path}}}
...
function Stop-TransferMonitor {
    ...
    try{if($entry.Process){$entry.Process.Refresh();if(-not $entry.Process.HasExited){Stop-Process -Id $entry.Process.Id -Force -ErrorAction SilentlyContinue}}}catch{}
    try{Remove-Item -LiteralPath ([string]$entry.ProgressPath) -Force -ErrorAction SilentlyContinue}catch{}
    $script:Monitors.Remove($Id)
}
function Start-TransferMonitor {
    ...
    $pidValue=[int](Get-Prop $State 'WorkerPid' 0);if($pidValue -le 0 -or -not(Test-ProcessAlive $pidValue)){return}
    $provider=[string](Get-Prop $State 'Provider' '');if($provider -notin @('Epic','GOG','Amazon')){return}
...
function Merge-TransferProgress {
    ...
    $entry=$script:Monitors[$Id];$progress=Read-Json ([string]$entry.ProgressPath)
    ...
    $phase=[string](Get-Prop $progress 'Phase' [string](Get-Prop $State 'Phase' 'Downloading'));if($phase -in @('Downloading','Installing')){Set-Prop $State 'Phase' $phase}
    ...
    Set-Prop $State 'TelemetryUpdated' ([string](Get-Prop $progress 'Updated' ''));Set-Prop $State 'TelemetrySource' $(if([bool]$entry.Estimated){'Estimated total size + observed throughput'}else{[string](Get-Prop $progress 'TelemetryKind' 'Observed throughput')});Set-Prop $State 'EtaEstimated' ([bool]$entry.Estimated);Set-Prop $State 'Message' ([string](Get-Prop $progress 'Message' ([string](Get-Prop $State 'Message' '')))
    $latest=Read-Json $StatePath
    if($null -eq $latest -or -not [bool](Get-Prop $latest 'Busy' $false)){if($null -ne $latest){return $latest};return $State}
...
function Read-TransferStates {
    ...
    foreach($file in @(Get-ChildItem -LiteralPath $transferRoot -Filter 'transfer-*.json' -File -ErrorAction SilentlyContinue)){
        $state=Read-Json $file.FullName;if($null -eq $state){continue};$id=Get-TransferId $file.FullName $state;Set-Prop $state 'TransferId' $id;Set-Prop $state 'StatePath' $file.FullName
        $busy=[bool](Get-Prop $state 'Busy' $false);$workerPid=[int](Get-Prop $state 'WorkerPid' 0);$mode=[string](Get-Prop $state 'Mode' '')
        if($busy -and $workerPid -gt 0 -and -not(Test-ProcessAlive $workerPid)){Set-Prop $state 'Busy' $false;Set-Prop $state 'Phase' 'Failed';Set-Prop $state 'Error' 'Provider worker ended unexpectedly.';Set-Prop $state 'Updated' ([DateTime]::UtcNow.ToString('o'));Write-AtomicJson $file.FullName $state;$busy=$false}
        if($busy -and $mode -in @('Install','Update')){Start-TransferMonitor $id $file.FullName $state;$state=Merge-TransferProgress $id $file.FullName $state}else{Stop-TransferMonitor $id}
...
function Prune-TransferStates {
    param([object[]]$States)
    $cutoff=[DateTime]::UtcNow.AddDays(-7);$completed=@($States|Where-Object{-not [bool](Get-Prop $_ 'Busy' $false)}|Sort-Object {try{[datetime]::Parse([string](Get-Prop $_ 'Updated' ''))}catch{[datetime]::MinValue}} -Descending);$keep=@{};$index=0
...
function Publish-TransferAggregate {
    ...
    $signature=ConvertTo-Json -InputObject ([object[]]$ordered) -Depth 16 -Compress;if($signature -ne $script:LastAggregateSignature){$script:LastAggregateSignature=$signature;Write-AtomicJson $aggregatePath ([pscustomobject]@{Transfers=[object[]]$ordered;Updated=[DateTime]::UtcNow.ToString('o')})}
    $active=@($ordered|Where-Object{[bool](Get-Prop $_ 'Busy' $false)})
    if($active.Count -gt 1){
        $latest=$active[0];$legacyUpdated=[string](Get-Prop $latest 'Updated' '');if(-not $legacyUpdated){$legacyUpdated=[DateTime]::UtcNow.ToString('o')}
        $legacy=[pscustomobject]@{Busy=$true;Provider='Multiple';Mode='Install';Phase='Downloading';Message=("{0} provider downloads/installations are active." -f $active.Count);Progress=-1;Error='';GameId='';GameName=("{0} active downloads" -f $active.Count);WorkerPid=0;StartedAt=[string](Get-Prop $latest 'StartedAt' '');Updated=$legacyUpdated;ActiveTransferCount=$active.Count}
    }elseif($active.Count -eq 1){$legacy=$active[0]}elseif($ordered.Count -gt 0){$legacy=$ordered[0]}else{$legacy=[pscustomobject]@{Busy=$false;Provider='';Mode='';Phase='Ready';Message='Direct game providers are ready.';Progress=-1;Error='';GameId='';GameName='';WorkerPid=0;Updated=[DateTime]::UtcNow.ToString('o');ActiveTransferCount=0}}
    $legacySignature=ConvertTo-Json -InputObject $legacy -Depth 16 -Compress;if($legacySignature -ne $script:LastLegacySignature){$script:LastLegacySignature=$legacySignature;Write-AtomicJson $legacyStatePath $legacy}
}

try{
    New-Item -ItemType Directory -Force -Path $providerRoot,$transferRoot|Out-Null
    while(Test-ProcessAlive $ParentPid){$states=@(Read-TransferStates);Prune-TransferStates $states;Publish-TransferAggregate $states;Start-Sleep -Milliseconds 650}
}finally{foreach($id in @($script:Monitors.Keys)){Stop-TransferMonitor ([string]$id)}}
