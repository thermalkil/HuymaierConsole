...
foreach($n in @('HUYMAIER_GPU_SHELF_ASSET_CACHE_V4','DefaultShelfTextureSize = 512','IsCacheCurrent','EnsureCompiled','DecodePixelWidth','DecodePixelHeight','TEXCOORD_0','COLOR_0','Cr, Cg, Cb, Ca','EmissiveTexture','MetallicRoughnessTexture','NormalTexture','OcclusionTexture','NormalScale','OcclusionStrength','LastWriteTimeUtc.Ticks')){if($compilerText.IndexOf($n,[StringComparison]::Ordinal)-lt0){throw "GPU shelf compiler v4 contract missing: $n"}}
...
$temp=Join-Path $(if($env:RUNNER_TEMP){$env:RUNNER_TEMP}else{[IO.Path]::GetTempPath()}) ('hc-gpu-cache-v4-'+[guid]::NewGuid().ToString('N'));New-Item -ItemType Directory -Force -Path $temp|Out-Null
...
function New-ProbeGlb([string]$Path){
    ...
    try{
        ...
        # Normalized UBYTE VEC4 is a valid glTF COLOR_0 encoding and catches both
        # accessor normalization and the v4 cache's RGBA vertex-color path.
        foreach($i in 0..3){$bw.Write([byte]156);$bw.Write([byte]8);$bw.Write([byte]219);$bw.Write([byte]255)}
    ...
    $json="{`"asset`":{`"version`":`"2.0`"},`"scene`":0,`"scenes`":[{`"nodes`":[0]}],`"nodes`":[{`"mesh`":0,`"translation`":[.25,0,0]}],`"meshes`":[{`"primitives`":[{`"attributes`":{`"POSITION`":0,`"NORMAL`":1,`"TEXCOORD_0`":2,`"COLOR_0`":3},`"indices`":4,`"material`":0,`"mode`":4}]}],`"materials`":[{`"pbrMetallicRoughness`":{`"baseColorTexture`":{`"index`":0},`"metallicFactor`":.2,`"roughnessFactor`":.7}}],`"samplers`":[{`"wrapS`":10497,`"wrapT`":10497}],`"textures`":[{`"sampler`":0,`"source`":0}],`"images`":[{`"bufferView`":5,`"mimeType`":`"image/png`"}],`"buffers`":[{`"byteLength`":$($bin.Length)}],`"bufferViews`":[{`"buffer`":0,`"byteOffset`":0,`"byteLength`":48},{`"buffer`":0,`"byteOffset`":48,`"byteLength`":48},{`"buffer`":0,`"byteOffset`":96,`"byteLength`":32},{`"buffer`":0,`"byteOffset`":128,`"byteLength`":16},{`"buffer`":0,`"byteOffset`":144,`"byteLength`":12},{`"buffer`":0,`"byteOffset`":$img,`"byteLength`":$($png.Length)}],`"accessors`":[{`"bufferView`":0,`"componentType`":5126,`"count`":4,`"type`":`"VEC3`"},{`"bufferView`":1,`"componentType`":5126,`"count`":4,`"type`":`"VEC3`"},{`"bufferView`":2,`"componentType`":5126,`"count`":4,`"type`":`"VEC2`"},{`"bufferView`":3,`"componentType`":5121,`"normalized`":true,`"count`":4,`"type`":`"VEC4`"},{`"bufferView`":4,`"componentType`":5123,`"count`":6,`"type`":`"SCALAR`"}]}"
...
try{
    ...
    $br=New-Object IO.BinaryReader([IO.File]::OpenRead($cache));try{
        $magic=-join$br.ReadChars(4);$version=$br.ReadInt32();$sourceLength=$br.ReadInt64();$ticks=$br.ReadInt64();$quality=$br.ReadInt32();$vc=$br.ReadInt32();$ic=$br.ReadInt32();$dc=$br.ReadInt32();$images=$br.ReadInt32();for($i=0;$i-lt6;$i++){[void]$br.ReadSingle()}
        if($magic-ne'HC3D'-or$version-ne4-or$quality-ne512){throw 'GPU shelf v4 cache header is invalid.'};if($vc-ne4-or$ic-ne6-or$dc-ne1-or$images-ne1){throw "GPU shelf cache counts unexpected: vertices=$vc indices=$ic draws=$dc images=$images"}
        $vertexStart=$br.BaseStream.Position;$br.BaseStream.Position=$vertexStart+80
        $cr=$br.ReadSingle();$cg=$br.ReadSingle();$cb=$br.ReadSingle();$ca=$br.ReadSingle()
        if([math]::Abs($cr-(156.0/255.0))-gt.01-or[math]::Abs($cg-(8.0/255.0))-gt.01-or[math]::Abs($cb-(219.0/255.0))-gt.01-or[math]::Abs($ca-1.0)-gt.001){throw "HC3D v4 lost normalized COLOR_0: $cr,$cg,$cb,$ca"}
        $br.BaseStream.Position=$vertexStart+($vc*96)+($ic*4)+($dc*136)
        $iw=$br.ReadInt32();$ih=$br.ReadInt32();$bytes=$br.ReadInt32();if($iw-ne512-or$ih-ne256){throw "Shelf texture was not downsampled to 512x256 (got ${iw}x${ih})."};if($bytes-ne$iw*$ih*4){throw 'GPU shelf cache texture is not packed BGRA32.'}
    }finally{$br.Dispose()}
    Write-Host 'platformModelGpuAssetCompilerGate: success';Write-Host 'platformModelGpuCacheFreshnessGate: success';Write-Host 'platformModelGpuTextureDownsampleGate: success';Write-Host 'platformModelGpuCacheGeometryGate: success';Write-Host 'platformModelGpuVertexColorCompileGate: success'
