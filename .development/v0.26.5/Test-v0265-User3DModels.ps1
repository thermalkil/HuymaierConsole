...
foreach($needle in @(
    'HUYMAIER_USER_3D_MODELS_RUNTIME_V2',
    "Join-Path `$script:DataDir '3D Models'",
    "Join-Path `$script:BaseDir '3D Models'",
    'README - Model Names.txt',
    'Get-HcDetectedUser3DModelCount',
    "`$script:SubPage -ne 'Customization'",
    "@('platform-visual-style','platform-icon-scale-slider','platform-model-scale-slider','open-3d-models-folder','3d-models-detected')",
    'Platform visuals: ',
    'Icon card size',
    '3D model size',
    '3D Models Folder — ',
    'Missing GLBs use the normal icon',
    'Start from the original icon card every time',
    'HcLiveModelCard',
    'Live 3D card host was not found',
    'Live 3D model view could not be created'
)){if(-not $text.Contains($needle)){throw "User 3D Models runtime contract missing: $needle"}}
