...
if(-not $cardScope.Contains('$visualHost=Get-HcPlatformVisualHost')){throw 'New-PlatformCard must use a non-reserved visualHost variable.'}
...
foreach($needle in @('$oldChild=$null','New-HcUserCardLiveModelView',"throw 'Live model view could not be created.'",'$visualHost.Child=$view','if($null -ne $oldChild){$visualHost.Child=$oldChild}','HcUser3DReadyCount++','HcUser3DFailedCount++')){if(-not $queueScope.Contains($needle)){throw "Lazy card queue does not preserve icon fallback and diagnostics around live attachment: $needle"}}
