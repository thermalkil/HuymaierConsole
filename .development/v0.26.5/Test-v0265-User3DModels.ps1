...
foreach($ps in @($runtime,$liveRuntime,$optimizer)){$tokens=$null;$errors=$null;$ast=[Management.Automation.Language.Parser]::ParseFile($ps,[ref]$tokens,[ref]$errors);if(@($errors).Count){throw "$ps failed Windows PowerShell 5.1 parse: $(@($errors|ForEach-Object{$_.Message}) -join '; ')"};if($ps -in @($runtime,$liveRuntime)){foreach($variable in @($ast.FindAll({param($node)$node -is [Management.Automation.Language.VariableExpressionAst]},$true))){if([string]::Equals([string]$variable.VariablePath.UserPath,'Host',[StringComparison]::OrdinalIgnoreCase)){throw "Reserved PowerShell automatic variable `$Host is referenced by active 3D runtime: $ps line $($variable.Extent.StartLineNumber)"}}}}
$text=Get-Content -Raw -LiteralPath $runtime -Encoding UTF8
foreach($needle in @('HUYMAIER_USER_3D_MODELS_RUNTIME_V4',"Join-Path `$script:DataDir '3D Models'","Join-Path `$script:BaseDir '3D Models'",'README - Model Names.txt','Get-HcDetectedUser3DModelCount',"`$script:SubPage-ne'Customization'",'Platform visuals: ','Icon card size','3D model size','3D Models Folder - ','Missing or failed GLBs keep the normal icon','HcPlatformPresentationOwner','HuymaierUser3DModelsV4','HcModelsBaseNewPlatformCard','HcModelsBaseAddPlatformRail','HcUserModelsBaseAdjustSelectedSlider','HcUserModelsBaseApplyControllerNavigation','Reset-HcUser3DCardQueue','Start-HcUser3DCardTimer','Queue-HcUser3DCard','Update-HcUser3DCardQueue','New-HcUserCardLiveModelView','HcLiveModelCard','GeometryCount','VertexCount','Loading live 3D model','3D platform presentation pass: owner=','VisualHost','Controller UI dispatch failed:')){if(-not $text.Contains($needle)){throw "User 3D Models runtime contract missing: $needle"}}
if($text.Contains('Set-HcAtlasFallbackVisual')){throw 'Final user 3D Models owner must not substitute the static atlas for missing GLBs.'}
...
foreach($needle in @('$button=& $script:HcUserModelsBaseNewPlatformCard','$visualHost=Get-HcPlatformVisualHost','Queue-HcUser3DCard')){if(-not $cardScope.Contains($needle)){throw "Final V4 card owner missing: $needle"}}
$queueStart=$text.IndexOf('function Update-HcUser3DCardQueue',[StringComparison]::Ordinal);$cardFn=$text.IndexOf('function New-PlatformCard',[math]::Max(0,$queueStart),[StringComparison]::Ordinal)
...
foreach($needle in @('$oldChild=$null','New-HcUserCardLiveModelView',"throw 'Live model view could not be created.'",'$visualHost.Child=$view','if($oldChild){$visualHost.Child=$oldChild}','HcUser3DReadyCount++','HcUser3DFailedCount++')){if(-not $queueScope.Contains($needle)){throw "Lazy card queue does not preserve icon fallback and diagnostics around live attachment: $needle"}}
...
if($actionStart-lt0-or$sliderStart-le$actionStart){throw 'Could not isolate V4 platform settings action owner.'};$actionScope=$text.Substring($actionStart,$sliderStart-$actionStart)
foreach($needle in @("'platform-visual-style'","'platform-icon-scale-slider'","'platform-model-scale-slider'","'open-3d-models-folder'",'Save-Config','Render-Page')){if(-not$actionScope.Contains($needle)){throw "V4 settings action owner missing: $needle"}}
$navStart=$text.IndexOf('function Apply-ControllerNavigation',[StringComparison]::Ordinal);if($navStart-lt0){throw 'V4 controller UI dispatch boundary is missing.'};$navScope=$text.Substring($navStart);foreach($needle in @('& $script:HcUserModelsBaseApplyControllerNavigation','Controller UI dispatch failed:')){if(-not$navScope.Contains($needle)){throw "V4 controller UI dispatch boundary missing: $needle"}}
