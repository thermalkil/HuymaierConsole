...
foreach($n in @('CacheVersion = 3','Determinant3x3','bool mirrored')){if($c.IndexOf($n,[StringComparison]::Ordinal)-lt0){throw "Mirrored-node winding contract missing: $n"}}
$compact=[regex]::Replace($c,'\s+','')
foreach($n in @('indices.Add((uint)(baseVertex+ix[i]));indices.Add((uint)(baseVertex+ix[i+2]));indices.Add((uint)(baseVertex+ix[i+1]));','if(mirrored)tw=-tw;')){if($compact.IndexOf($n,[StringComparison]::Ordinal)-lt0){throw "Mirrored-node winding/tangent contract missing: $n"}}
