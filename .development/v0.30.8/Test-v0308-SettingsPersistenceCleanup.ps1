...
$model=Read-Hc 'HuymaierConsoleModelPresentation.ps1'
$bootstrap=Read-Hc 'HuymaierBootstrap.ps1'
...
try{
    ...
    Require ([string]$merged.Known-eq'persisted') 'Known property did not merge.'
    Require ([int]$merged.ConsoleBrightness-eq170) 'ConsoleBrightness did not merge.'
    Require ($null-ne$merged.PSObject.Properties['SyntheticModuleSetting']) 'Unknown module-owned property was dropped.'
    Require (Write-HcConfigAtomic -Path $path -Config $merged -Depth 16) 'Atomic settings write did not report success.'
    $roundTrip=Read-HcPersistedConfig -Path $path
    Require ([int]$roundTrip.ConsoleBrightness-eq170) 'ConsoleBrightness did not survive round trip.'
    Require ([string]$roundTrip.TheGamesDbApiKey-eq'tgdb-test-key') 'TheGamesDB key did not survive round trip.'
    Require ([string]$roundTrip.SteamGridDbApiKey-eq'sgdb-test-key') 'SteamGridDB key did not survive round trip.'
    Require ([string]$roundTrip.SyntheticModuleSetting.Mode-eq'PersistMe') 'Synthetic module-owned setting did not survive round trip.'
    $view=@($roundTrip.PlatformModelDefaultViews)[0]
    foreach($pair in @(@('LightPercent',240),@('KeyLightPercent',330),@('LightAzimuth',27),@('LightElevation',41),@('LightTemperature',7200),@('AmbientPercent',180),@('SpecularPercent',260),@('HighlightSizePercent',175),@('FanPercent',50))){Require ([int]$view.($pair[0]) -eq [int]$pair[1]) ("Advanced model setting did not survive: "+$pair[0])}
    Require (@(Get-ChildItem -LiteralPath $temp -File -Filter '*.tmp' -ErrorAction SilentlyContinue).Count-eq0) 'Atomic settings writer leaked temporary files.'
    Require (-not(Test-Path -LiteralPath ($path+'.replace-backup'))) 'Atomic settings writer leaked replacement backup.'
}finally{Remove-Item -LiteralPath $temp -Recurse -Force -ErrorAction SilentlyContinue}

Write-Host 'v0.30.8 settings persistence/model auto-save/production cleanup validation passed.'
