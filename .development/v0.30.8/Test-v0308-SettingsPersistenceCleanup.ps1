...
$core=Read-Hc 'HuymaierConsole.ps1';$store=Read-Hc 'HuymaierSettingsStore.ps1';$custom=Read-Hc 'HuymaierCustomization.ps1';$model=Read-Hc 'HuymaierConsoleModelPresentation.ps1';$bootstrap=Read-Hc 'HuymaierBootstrap.ps1';$installer=Read-Hc 'Install-HuymaierConsole.ps1';$installerCore=Read-Hc 'HuymaierInstallerCore.ps1'
Require ($core.Contains('HUYMAIER_V0308_SETTINGS_STORE_CORE_V4')) 'Core settings-store marker is missing.'
...
Require ($model.Contains('HUYMAIER_V0308_MODEL_SETTINGS_AUTOSAVE_V4')) 'Model editor auto-save marker missing.'
...
Require ($installerCore.Contains('HUYMAIER_V0308_SETTINGS_STORE_REQUIRED_V4')) 'Installer required payload omits settings store.'
foreach($retired in @('HuymaierV0262Hardening.ps1','HuymaierV0262ProviderRuntime.ps1','HuymaierV0262Runtime.ps1')){Require (-not(Test-Path -LiteralPath (Join-Path $root $retired))) "Retired production runtime survived cleanup: $retired"}
