...
# The materializer is intentionally idempotent. A prior CI run may have already
# committed one half of this integration while a concurrent documentation/source
# sync advanced the branch. Re-running must finish only the missing pieces.
p=p.replace('Huymaier-Console/0.26.3','Huymaier-Console/0.26.4')

# Verify GitHub's release-asset digest whenever the API exposes a sha256 digest.
digest_marker="if($asset.PSObject.Properties['digest'] -and [string]$asset.digest -match '(?i)^sha256:(?<hash>[0-9a-f]{64})$')"
if digest_marker not in p:
    anchor="        Invoke-WebRequest -UseBasicParsing -Headers $headers -Uri ([string]$asset.browser_download_url) -OutFile $archive\n"
    if p.count(anchor)!=1:
        raise SystemExit(f'GitHub release digest insertion anchor: expected one match, found {p.count(anchor)}')
    block=(
        anchor+
...
# data/configs/saves can legitimately be stored beside the executable.
...
function Install-MednafenLatest {
    ...
    $links=[regex]::Matches($html,'(?i)href=["''](?<url>[^"'']*mednafen-(?<version>[0-9]+(?:\.[0-9]+)+)-win64\.zip)["'']')
    if($links.Count -eq 0){throw 'The current official Mednafen 64-bit Windows release could not be identified.'}
...
'''
if "'3DS' {$exe=Install-GithubArchive 'azahar-emu/azahar'" not in p:
...
if 'string azaharData=settings.emulatorDataPath;' not in c:
...
'''
    if c.count(old)!=1: raise SystemExit(f'Wave1 save-root insertion anchor: expected one match, found {c.count(old)}')
    c=c.replace(old,new,1)
cs.write_text(c,encoding='utf-8')
print('Wave 1 latest-emulator install and storage roots are complete and idempotent')
