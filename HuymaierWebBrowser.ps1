...
# Define all browser state before StrictMode can observe it. WebView2 itself
# remains lazy; these assignments do not construct or load the browser runtime.
...
function Test-HcNativeBrowserAvailable {
    ...
    return (Test-Path -LiteralPath $core -PathType Leaf) -and
           (Test-Path -LiteralPath $wpf -PathType Leaf) -and
...
function Import-HcWebView2 {
    ...
    $wpf = Join-Path $sdk 'Microsoft.Web.WebView2.Wpf.dll'
    $loader = Join-Path $sdk 'WebView2Loader.dll'
    if (-not (Test-Path -LiteralPath $core) -or
        -not (Test-Path -LiteralPath $wpf) -or
        -not (Test-Path -LiteralPath $loader)) {
        return $false
    }

    try {
        if (-not (($env:PATH -split ';') -contains $sdk)) {
...
function New-HcBrowserButton {
    ...
    $button.Background = '#C4162234'
    $button.BorderBrush = '#465B76'
    ...
    $button.Add_Click({
        ...
        try {
            Invoke-HcBrowserToolbarAction ([string]$sender.Tag)
        } catch {
            Write-Log "Browser toolbar action failed: $($_.Exception.Message)" 'WARN'
        }
    })
    ...
    $button.Add_MouseEnter({
        ...
        for ($i = 0; $i -lt $script:HcBrowserToolbarItems.Count; $i++) {
            if ($script:HcBrowserToolbarItems[$i].Control -eq $sender) {
                $script:HcBrowserToolbarIndex = $i
...
function Add-HcBrowserToolbarItem {
    ...
    $script:HcBrowserToolbarItems += ,$item
    if ($Control -is [System.Windows.Controls.Button]) {
        $script:HcBrowserToolbarButtons += $Control
    }
}
...
function Initialize-HuymaierWebBrowser {
    ...
    # Startup only keeps the provider-auth bridge alive. Loading WebView2 and
    # constructing the browser UI is deferred until Web/provider auth needs it.
    ...
    if ($null -eq $script:HcBrowserAuthTimer) {
        ...
        $timer.Add_Tick({
            try {
                Poll-HcBrowserAuthRequest
            } catch {
                Write-Log "Browser auth bridge: $($_.Exception.Message)" 'WARN'
            }
        })
        $timer.Start()
    ...
    $toolbarBorder.Background = '#FF0B111C'
    $toolbarBorder.BorderBrush = '#30435D'
    $toolbarBorder.BorderThickness = '0,0,0,1'
    $toolbarBorder.Padding = '18,12'
    [System.Windows.Controls.Grid]::SetRow($toolbarBorder,0)
    $overlay.Children.Add($toolbarBorder) | Out-Null

    $toolbar = New-Object System.Windows.Controls.Grid
    foreach ($width in @('Auto','Auto','Auto','Auto','*','Auto')) {
        $toolbar.ColumnDefinitions.Add((New-Object System.Windows.Controls.ColumnDefinition -Property @{Width=$width}))
    }
    ...
    for ($i = 0; $i -lt $definitions.Count; $i++) {
        ...
        $button = New-HcBrowserButton ([string]$definition[0]) ([string]$definition[1]) ([double]$definition[2])
        [System.Windows.Controls.Grid]::SetColumn($button,$i)
        $toolbar.Children.Add($button) | Out-Null
        Add-HcBrowserToolbarItem $button ([string]$definition[0])
    }

    $address = New-Object System.Windows.Controls.Border
    $address.Margin = '10,0,10,0'
    $address.Padding = '16,8'
    $address.Background = '#FF111B29'
    $address.BorderBrush = '#3C526D'
    $address.BorderThickness = '1'
    $address.CornerRadius = 11
    $address.Cursor = 'Hand'
    [System.Windows.Controls.Grid]::SetColumn($address,4)
    $toolbar.Children.Add($address) | Out-Null

    $addressText = New-Object System.Windows.Controls.TextBlock
    $addressText.Text = 'Search or enter address'
    $addressText.Foreground = '#D8E2EF'
    $addressText.FontSize = 15
    $addressText.TextTrimming = 'CharacterEllipsis'
    $addressText.VerticalAlignment = 'Center'
    $address.Child = $addressText

    $address.Add_MouseLeftButtonUp({ Show-HcBrowserAddressKeyboard })
    $address.Add_MouseEnter({
        if (Test-HcMouseHoverAllowed) {
            $script:HcBrowserToolbarIndex = 4
            $script:HcBrowserFocusArea = 'Toolbar'
            Update-HcBrowserToolbarVisuals
        }
    ...
    $closeButton = New-HcBrowserButton 'Close' 'CLOSE' 82
    [System.Windows.Controls.Grid]::SetColumn($closeButton,5)
    $toolbar.Children.Add($closeButton) | Out-Null
    Add-HcBrowserToolbarItem $closeButton 'Close'

    $browserHost = New-Object System.Windows.Controls.Grid
    $browserHost.Background = '#FF11151C'
    $browserHost.ClipToBounds = $true
    [System.Windows.Controls.Grid]::SetRow($browserHost,1)
    $overlay.Children.Add($browserHost) | Out-Null

    $fallback = New-Object System.Windows.Controls.StackPanel
    $fallback.HorizontalAlignment = 'Center'
    $fallback.VerticalAlignment = 'Center'
    $fallback.MaxWidth = 780

    $fallbackTitle = New-Object System.Windows.Controls.TextBlock
    $fallbackTitle.Text = 'Native browser is preparing'
    $fallbackTitle.FontSize = 30
    $fallbackTitle.FontWeight = 'Bold'
    $fallbackTitle.Foreground = 'White'
    $fallbackTitle.TextAlignment = 'Center'
    $fallback.Children.Add($fallbackTitle) | Out-Null

    $fallbackText = New-Object System.Windows.Controls.TextBlock
    $fallbackText.Text = 'WebView2 could not be initialized. Re-run the installer while online, then restart Huymaier Console.'
    $fallbackText.Margin = '0,14,0,0'
    $fallbackText.FontSize = 17
    $fallbackText.Foreground = '#AEBBD0'
    $fallbackText.TextWrapping = 'Wrap'
    $fallbackText.TextAlignment = 'Center'
    $fallback.Children.Add($fallbackText) | Out-Null
    $browserHost.Children.Add($fallback) | Out-Null

    $footer = New-Object System.Windows.Controls.Border
    $footer.Background = '#FF0A101A'
    $footer.BorderBrush = '#30435D'
    $footer.BorderThickness = '0,1,0,0'
    $footer.Padding = '18,11'
    [System.Windows.Controls.Grid]::SetRow($footer,2)
    $overlay.Children.Add($footer) | Out-Null

    $footerText = New-Object System.Windows.Controls.TextBlock
    $footerText.Foreground = '#D4DEEB'
    $footerText.FontSize = 15
    $footerText.FontWeight = 'SemiBold'
    $footerText.HorizontalAlignment = 'Center'
    $footer.Child = $footerText

    $root.Children.Add($overlay) | Out-Null
    $script:HcBrowserOverlay = $overlay
    $script:HcBrowserHost = $browserHost
    $script:HcBrowserFallback = $fallback
    $script:HcBrowserAddressBorder = $address
    $script:HcBrowserAddressText = $addressText
    $script:HcBrowserFooterText = $footerText

    if (Import-HcWebView2) {
        try {
            $web = New-Object Microsoft.Web.WebView2.Wpf.WebView2
            $web.HorizontalAlignment = 'Stretch'
            $web.VerticalAlignment = 'Stretch'
            ...
            $web.Add_CoreWebView2InitializationCompleted({
                param($sender,$eventArgs)
                Initialize-HcBrowserCore $sender $eventArgs
            })
            $browserHost.Children.Add($web) | Out-Null
            $script:HcBrowserWebView = $web
            $web.Source = [uri]'about:blank'
        } catch {
            $fallbackTitle.Text = 'Native browser unavailable'
            $fallbackText.Text = $_.Exception.Message
...
function Initialize-HcBrowserCore {
    ...
    if (-not $EventArgs.IsSuccess) {
        $script:HcBrowserFallback.Visibility = 'Visible'
        Write-Log "WebView2 initialization failed: $($EventArgs.InitializationException.Message)" 'ERROR'
        return
    }
    ...
    try {
        ...
        $core.Add_NewWindowRequested({
            ...
            try {
                $e.Handled = $true
                if ($e.Uri) { $script:HcBrowserWebView.Source = [uri]$e.Uri }
            } catch { }
        })
        ...
        $Sender.Add_NavigationStarting({
            param($s,$e)
            try { $script:HcBrowserAddressText.Text = [string]$e.Uri } catch { }
        })
        $Sender.Add_NavigationCompleted({
            param($s,$e)
            try {
                Update-HcBrowserAddress
                Install-HcBrowserControllerScript
                Test-HcBrowserAuthenticationCompletion
            } catch {
                Write-Log "Native browser navigation handler: $($_.Exception.Message)" 'WARN'
            }
        })

        $script:HcBrowserReady = $true
        $script:HcBrowserFallback.Visibility = 'Collapsed'
        try {
            ...
            $tmp = "$script:HcBrowserReadyPath.tmp"
            $ready | ConvertTo-Json -Compress | Set-Content -LiteralPath $tmp -Encoding UTF8
            Move-Item -LiteralPath $tmp -Destination $script:HcBrowserReadyPath -Force
        } catch { }
        Write-Log 'Native WebView2 controller browser initialized.'
    } catch {
        Write-Log "Native browser core setup failed: $($_.Exception.Message)" 'ERROR'
    }
}
...
function Update-HcBrowserAddress {
    ...
    try {
        $url = [string]$script:HcBrowserWebView.Source.AbsoluteUri
        if ($url -eq 'about:blank') { $url = 'Search or enter address' }
        $script:HcBrowserAddressText.Text = $url
    } catch { }
}

function Get-HcBrowserRequestFromDisk {
    if (-not $script:HcBrowserAuthRequestPath) { return $null }
    if (-not (Test-Path -LiteralPath $script:HcBrowserAuthRequestPath -PathType Leaf)) { return $null }
    try {
        return Get-Content -Raw -LiteralPath $script:HcBrowserAuthRequestPath | ConvertFrom-Json
    } catch {
        return $null
    }
}
...
function Test-HcBrowserRequestOwnerAlive {
    ...
    try {
        Get-Process -Id $workerProcessId -ErrorAction Stop | Out-Null
        return $true
    } catch {
        return $false
    }
...
function Write-HcBrowserAuthResult {
    ...
    try {
        ...
        Write-Log "Unable to write browser authorization result: $($_.Exception.Message)" 'WARN'
    }
}
...
function Cancel-HcBrowserAuthentication {
    ...
    try {
        if ($null -ne $script:HcBrowserWebView) { $script:HcBrowserWebView.Source = [uri]'about:blank' }
    } catch { }
...
function Set-HcBrowserFocusArea {
    ...
    if ($Area -eq 'Toolbar') {
        if ($script:HcBrowserToolbarIndex -lt 0 -or
            $script:HcBrowserToolbarIndex -ge $script:HcBrowserToolbarItems.Count) {
            $script:HcBrowserToolbarIndex = 4
        }
    } else {
        try {
            if ($null -ne $script:HcBrowserWebView) { $script:HcBrowserWebView.Focus() | Out-Null }
        } catch { }
    }
    Update-HcBrowserToolbarVisuals
}
...
function Update-HcBrowserToolbarVisuals {
    ...
    for ($i = 0; $i -lt $script:HcBrowserToolbarItems.Count; $i++) {
        ...
        $active = $script:HcBrowserActive -and
                  $script:HcBrowserFocusArea -eq 'Toolbar' -and
                  $i -eq $script:HcBrowserToolbarIndex
        try {
            if ($active) {
                $control.BorderBrush = '#FFF0CC58'
                $control.BorderThickness = '3'
                $control.Background = '#FF25344A'
            } else {
                $control.BorderBrush = '#465B76'
                $control.BorderThickness = '1'
                $control.Background = '#C4162234'
            }
        } catch { }
    }
    ...
    if ($null -ne $script:HcBrowserFooterText) {
        if ($script:HcBrowserFocusArea -eq 'Toolbar') {
            $script:HcBrowserFooterText.Text = 'TOP BAR   Left/Right Move    A Select    X Type    Down Web    Y Web    B Back    GUIDE Main Menu'
        } else {
            $script:HcBrowserFooterText.Text = 'WEB   D-PAD Navigate    A Select    X Type    Y Top Bar    B Back    LB/RB History    GUIDE Main Menu'
        }
    }
}
...
function ConvertTo-HcBrowserDestination {
    ...
    if ([string]::IsNullOrWhiteSpace($value)) { return 'https://www.google.com' }
...
function Open-HuymaierBrowser {
    ...
    $script:HcBrowserActive = $true
    $script:HcBrowserToolbarIndex = 4
    $script:HcBrowserOverlay.Visibility = 'Visible'
    Set-HcBrowserFocusArea 'Toolbar'

    try {
        if ($Title) { $script:HcBrowserFooterText.ToolTip = $Title }
    } catch { }
    try { Stop-PlatformBackgroundAnimations } catch { }

    try {
        if ($null -ne $script:HcBrowserWebView) {
            $script:HcBrowserWebView.Visibility = 'Visible'
            $script:HcBrowserWebView.Source = [uri]$Url
        }
    } catch {
        Set-ConsoleNotice "Unable to open the native browser: $($_.Exception.Message)" 'ERROR'
    }
    Update-HcBrowserToolbarVisuals
    Update-Footer
}
...
function Close-HuymaierBrowser {
    ...
    $script:HcBrowserActive = $false
    $script:HcBrowserOverlay.Visibility = 'Collapsed'
    $script:HcBrowserFocusArea = 'Web'
    try {
        if ($null -ne $script:HcBrowserWebView) { $script:HcBrowserWebView.Visibility = 'Visible' }
    } catch { }
    Update-HcBrowserToolbarVisuals
    Update-Footer
}
...
function Invoke-HcBrowserToolbarAction {
    ...
    if ($Action -eq 'Address') {
        Show-HcBrowserAddressKeyboard
        return
    }

    if ($Action -eq 'Close') {
        if ($null -ne $script:HcBrowserAuthRequest -or
            (Test-Path -LiteralPath $script:HcBrowserAuthRequestPath -PathType Leaf)) {
            Cancel-HcBrowserAuthentication
        } else {
            Close-HuymaierBrowser
        }
        return
    ...
    switch ($Action) {
        'Back' {
            try {
                if ($script:HcBrowserWebView.CanGoBack) {
                    $script:HcBrowserWebView.GoBack()
                } elseif ($null -ne $script:HcBrowserAuthRequest -or
                          (Test-Path -LiteralPath $script:HcBrowserAuthRequestPath -PathType Leaf)) {
                    Cancel-HcBrowserAuthentication
                } else {
                    Close-HuymaierBrowser
                ...
                Write-Log "Browser back action failed: $($_.Exception.Message)" 'WARN'
            }
        }
        'Forward' {
            try {
                if ($script:HcBrowserWebView.CanGoForward) { $script:HcBrowserWebView.GoForward() }
            } catch { }
        }
        'Reload' {
            try { $script:HcBrowserWebView.Reload() } catch { }
        }
        'Home' {
            Open-HuymaierBrowser 'https://www.google.com' 'Web'
        }
    }
}
...
function Show-HcBrowserAddressKeyboard {
    ...
    $current = ''
    try { $current = [string]$script:HcBrowserWebView.Source.AbsoluteUri } catch { }
    if ($current -eq 'about:blank') { $current = '' }
...
function Invoke-HcBrowserScriptAsync {
    ...
    if (-not $script:HcBrowserReady -or
        $null -eq $script:HcBrowserWebView -or
        $null -eq $script:HcBrowserWebView.CoreWebView2) {
        return
    }
    ...
    try {
        ...
        $tick = {
            ...
            $timer.Stop()
            try {
                & $callback ([string]$operation.Result)
            } catch {
                Write-Log "Browser script callback: $($_.Exception.Message)" 'WARN'
            }
        }.GetNewClosure()
        $timer.Add_Tick($tick)
        $timer.Start()
    } catch {
        Write-Log "Browser script execution failed: $($_.Exception.Message)" 'WARN'
    }
}
...
function Install-HcBrowserControllerScript {
    $scriptText = @'
(()=>{
if(window.__hcControllerInstalled)return;
window.__hcControllerInstalled=true;
const style=document.createElement('style');
style.id='hc-controller-style';
style.textContent=`.hc-controller-focus{outline:4px solid #f0cc58!important;outline-offset:3px!important;box-shadow:0 0 0 5px rgba(0,0,0,.65),0 0 22px rgba(240,204,88,.7)!important}`;
...
window.__hcSet=e=>{document.querySelectorAll('.hc-controller-focus').forEach(x=>x.classList.remove('hc-controller-focus'));if(e){e.classList.add('hc-controller-focus');try{e.focus({preventScroll:true})}catch(_){e.focus()}e.scrollIntoView({block:'center',inline:'nearest',behavior:'smooth'});}return !!e};
...
function Open-HcBrowserEditableKeyboard {
    ...
    if ($fallbackToAddress) {
        $js = 'JSON.stringify(window.__hcInput?window.__hcInput():{editable:false})'
    } else {
        $js = 'JSON.stringify(window.__hcActivate?window.__hcActivate():{editable:false})'
    }
    ...
    $completed = {
        ...
        try {
            ...
            if ($null -ne $info -and [bool]$info.editable) {
                ...
                $title = 'Enter text'
                $initialText = [string]$info.value
                $mode = 'BrowserInput'
                if ($secure) {
                    $title = 'Enter password'
                    $initialText = ''
                    $mode = 'BrowserInputSecure'
                }
                Show-NativeKeyboard -Title $title -InitialText $initialText -Mode $mode -Context $null
            ...
            if ($fallbackToAddress) {
                try { Show-HcBrowserAddressKeyboard } catch { }
            }
        }
    }.GetNewClosure()
...
function Handle-HcBrowserController {
    ...
    if ($Direction) {
        if ($Direction -ne $script:LastDirection -or $now -ge $script:NextDirectionAt) {
            if ($script:HcBrowserFocusArea -eq 'Toolbar') {
                switch ($Direction) {
                    'Left' {
                        $script:HcBrowserToolbarIndex--
                        if ($script:HcBrowserToolbarIndex -lt 0) {
                            $script:HcBrowserToolbarIndex = $script:HcBrowserToolbarItems.Count - 1
                        }
                    }
                    'Right' {
                        $script:HcBrowserToolbarIndex++
                        if ($script:HcBrowserToolbarIndex -ge $script:HcBrowserToolbarItems.Count) {
                            $script:HcBrowserToolbarIndex = 0
                        }
                    }
    ...
    if (Is-NewButtonPress $Mask 4) {
        if ($script:HcBrowserFocusArea -eq 'Toolbar') {
            if ($script:HcBrowserToolbarItems.Count -gt 0) {
                $action = [string]$script:HcBrowserToolbarItems[$script:HcBrowserToolbarIndex].Action
                Invoke-HcBrowserToolbarAction $action
            }
        } else {
            Open-HcBrowserEditableKeyboard
        }
    }
    ...
    if (Is-NewButtonPress $Mask 1) {
        $script:HcBrowserToolbarIndex = 4
        Set-HcBrowserFocusArea 'Toolbar'
    }
    if (Is-NewButtonPress $Mask 2) {
        if (Get-Command Show-HcMainMenu -ErrorAction SilentlyContinue) { Show-HcMainMenu }
    }
...
function Handle-HcBrowserKey {
    ...
    switch ([string]$Key) {
        'Left' {
            if ($script:HcBrowserFocusArea -eq 'Toolbar') {
                $script:HcBrowserToolbarIndex--
                if ($script:HcBrowserToolbarIndex -lt 0) { $script:HcBrowserToolbarIndex = $script:HcBrowserToolbarItems.Count - 1 }
                Update-HcBrowserToolbarVisuals
            } else {
                Navigate-HcBrowser 'Left'
            }
        }
        'Right' {
            if ($script:HcBrowserFocusArea -eq 'Toolbar') {
                $script:HcBrowserToolbarIndex++
                if ($script:HcBrowserToolbarIndex -ge $script:HcBrowserToolbarItems.Count) { $script:HcBrowserToolbarIndex = 0 }
                Update-HcBrowserToolbarVisuals
            } else {
                Navigate-HcBrowser 'Right'
            }
        }
        'Up' {
            if ($script:HcBrowserFocusArea -eq 'Web') { Navigate-HcBrowser 'Up' }
        }
        'Down' {
            if ($script:HcBrowserFocusArea -eq 'Toolbar') {
                Set-HcBrowserFocusArea 'Web'
            } else {
                Navigate-HcBrowser 'Down'
            }
        }
        'Enter' {
            if ($script:HcBrowserFocusArea -eq 'Toolbar') {
        ...
        'Space' { Open-HcBrowserEditableKeyboard }
...
function Poll-HcBrowserAuthRequest {
    if (-not (Test-Path -LiteralPath $script:HcBrowserAuthRequestPath -PathType Leaf)) {
        if ($null -ne $script:HcBrowserAuthRequest) {
            $script:HcBrowserAuthRequest = $null
            $script:HcBrowserAuthRequestId = ''
            $script:HcBrowserCompletionProbeBusy = $false
        }
    ...
    $script:HcBrowserAuthRequestId = $requestId
    $script:HcBrowserAuthRequest = $request
    $script:HcBrowserCompletionProbeBusy = $false

    $title = [string]$request.Title
    if (-not $title) { $title = 'Account sign-in' }
...
function Test-HcBrowserAuthenticationCompletion {
    ...
    $url = ''
    try { $url = [string]$script:HcBrowserWebView.Source.AbsoluteUri } catch { }

    if ($type -eq 'Callback') {
        $prefix = [string]$script:HcBrowserAuthRequest.CallbackPrefix
        if ($prefix -and $url.StartsWith($prefix,[StringComparison]::OrdinalIgnoreCase)) {
            Complete-HcBrowserAuthentication 'callback'
            return
        }
    }
    ...
    if ($type -eq 'UrlCode') {
        if ($url -match '[?&]code=([^&#]+)') {
            Complete-HcBrowserAuthentication ([uri]::UnescapeDataString([string]$matches[1]))
            return
        }
    }

    if ($type -eq 'AmazonAuthorizationCode') {
        if ($url -match '[?&]openid\.oa2\.authorization_code=([^&#]+)') {
            Complete-HcBrowserAuthentication ([uri]::UnescapeDataString([string]$matches[1]))
            return
        }
    }
        ...
        Invoke-HcBrowserScriptAsync '(document.body&&document.body.innerText)||""' {
            ...
            try {
                ...
                try {
                    ...
                    if ($text -match '"authorizationCode"\s*:\s*"([^"]+)"') {
...
function Stop-HuymaierWebBrowser {
    try {
        if ($null -ne $script:HcBrowserAuthTimer) { $script:HcBrowserAuthTimer.Stop() }
    } catch { }
    ...
    try {
        if ($null -ne $script:HcBrowserAuthRequest -or
            ...
            if ($null -eq $request) { $request = Get-HcBrowserRequestFromDisk }
            if ($null -ne $request) {
                Write-HcBrowserAuthResult -Request $request -Error 'Huymaier Console closed before sign-in completed.'
            }
    ...
    try {
        if ($script:HcBrowserReadyPath -and (Test-Path -LiteralPath $script:HcBrowserReadyPath -PathType Leaf)) {
            Remove-Item -LiteralPath $script:HcBrowserReadyPath -Force -ErrorAction SilentlyContinue
        }
    } catch { }

    try {
        if ($null -ne $script:HcBrowserWebView) { $script:HcBrowserWebView.Dispose() }
    } catch { }
    $script:HcBrowserReady = $false
}
